# Auto Stream Recording Twitch v1.6.0 https://github.com/EnterGin/Auto-Stream-Recording-Twitch
...
class TwitchRecorder:
    def __init__(self):
        # Configuration
        self.client_id: str = "kimne78kx3ncx6brgo4mv6wki5h1ko"  # Don't change this

        # path to ffmpeg.exe. Leave blank if Linux or ffmpeg in env PATH
        self.ffmpeg_path: str = r"D:\\twitch"

        # Time between checking (1.0 is recommended)
        self.refresh: float = 1.0

        # path to recorded and processed streams
        self.root_path: str = r"D:\\twitch"

        # name of timezone (list of timezones: https://stackoverflow.com/questions/13866926/is-there-a-list-of-pytz-timezones)
        self.timezoneName: str = "US/Eastern"

        # False - disable chat downloading, True - enable chat downloading
        self.chatdownload: bool = False

        # Windows: 0 - not minimazed cmd close after processing, 1 - minimazed cmd close after processing, 2 - minimazed cmd don't close after processing, 3 - no terminal, do in background
        ...
        # False - disable VOD downloading after stream's ending, True - enable VOD downloading after stream's ending
        self.downloadVOD: bool = False

        # False - disable rerun deleting, True - enable rerun deleting
        self.rerun_delete: bool = True

        # 0 - always ask to delete previous processed streams from recorded folder, 1 - don't ask, don't delete, 2 - don't ask, delete
        self.dont_ask_to_delete: int = 0

        # False - don't make folders for each processed stream, True - make folders for each processed stream
        self.make_stream_folder: bool = True

        # False - only date in processed VOD folder, True - date, title, game and username in processed VOD folder
        self.short_folder: bool = False

        # 1-10 for live stream, it's possible to use multiple threads to potentially increase the throughput. 2-3 is enough
        self.hls_segments: int = 3

        # 1-10 for downloading vod, it's possible to use multiple threads to potentially increase the throughput
        self.hls_segmentsVOD: int = 10

        # False - don't show streamlink debug, True - show streamlink debug
        self.streamlink_debug: bool = False

        # False - don't show warning windows (warnings will only be printed in terminal), True - show warning windows
        self.warning_windows: bool = True
    ...
    def run(self):
        ...
        if sys.platform.startswith("win32"):
            self.osCheck = SystemPlatform.WINDOWS  # 0
        elif sys.platform.startswith("linux"):
            self.osCheck = SystemPlatform.LINUX  # 1
            if self.cmdstate == 3:
                self.cmdstate = 2
        else:
        ...
        # cmdstatecommand
        if self.osCheck == SystemPlatform.WINDOWS:
            if self.cmdstate == 2:
        ...
        self.timezone = (
            pytz.timezone(self.timezoneName)
            .localize(datetime.datetime.now())
            .tzinfo._utcoffset.seconds
            / 60
            / 60
        )
        ...
        try:
            video_list = [
                f
                for f in os.listdir(self.recorded_path)
            ...
            for f in video_list:
                ...
                if self.short_folder:
                    dirname = f[shorta:shortb]
                    dirname = "".join(
                        x
                        for x in dirname
                        if x.isalnum() or not x in ["/", "\\", ":", "?", "*", '"', ">", "<", "|"]
                    )
                else:
                ...
                if self.make_stream_folder:
                    stream_dir_path = self.processed_path + "/" + dirname
                else:
                    stream_dir_path = self.processed_path

                if len(os.path.join(stream_dir_path, f)) >= 260:
                    if self.warning_windows:
                        if self.osCheck == SystemPlatform.WINDOWS:
                            subprocess.call(
                                self.main_cmd_window
                                + [
                                    "echo",
                                    "Path to stream is too long. (Max path length is 259 symbols) Stream will not be processed, please check root path.",
                                ]
                            )
                        else:
                            subprocess.call(
                                " ".join(
                                    ...
                                    + [
                                        ...
                                        "-c",
                                        "'" + "echo",
                                        '"Path to stream is too long. (Max path length is 259 symbols) Stream will not be processed, please check root path."; exec bash'
                                        + "'",
                                    ]
                                ),
                                shell=True,
                ...
                if os.path.isdir(stream_dir_path) is False and too_long_path == 0:
                    ...
                    try:
                        if self.ffmpeg_path != "" and self.osCheck == SystemPlatform.WINDOWS:
                            ...
                            if self.cmdstate == 3:
                                subprocess.Popen(
                                    [
                                        ...
                                        "-bsf:a",
                                        "aac_adtstoasc",
                                        os.path.join(stream_dir_path, f),
                                    ],
                                    stdout=None,
                                    stderr=None,
                                )
                    ...
                    os.path.exists(os.path.join(stream_dir_path, f)) is False and too_long_path == 0
                ):
                    print("Fixing " + recorded_filename + ".")
                    try:
                        if self.ffmpeg_path != "" and self.osCheck == SystemPlatform.WINDOWS:
        ...
        while True:
            ...
            if status == 1:
                ...
                print(
                    ...
                    " ",
                    "unexpected error. Try to check internet connection or client-id. Will try again in",
                    self.refresh,
                    "seconds.",
                )
                time.sleep(self.refresh)
                ...
                print(
                    datetime.datetime.now().strftime("%Hh%Mm%Ss"),
                    " ",
                    "Error while getting channel id. Try to check internet connection or client-id. Will try again in",
                    self.refresh,
                    "seconds.",
                )
                time.sleep(self.refresh)
                        ...
                        if self.warning_windows:
                            if self.osCheck == SystemPlatform.WINDOWS:
                                subprocess.call(
                                    ...
                                    + [
                                        "echo",
                                        "Rerun detected. Recorded file will "
                                        + rerun_message
                                        + "Please check streams to ensure that it was not a live stream.",
                                    ]
                                )
                            else:
                                subprocess.call(
                                    " ".join(
                                        ...
                                        + [
                                            ...
                                            '"Rerun detected. Recorded file will '
                                            + rerun_message
                                            + 'Please check streams to ensure that it was not a live stream."; exec bash'
                                            + "'",
                                        ]
                                    ),
                                    shell=True,
                    ...
                    if os.path.exists(recorded_filename) is True and rerun == 0:
                        try:
                            ...
                            if vodsinfodic["_total"] > 0:
                                ...
                                vod_date = datetime.datetime(
                                    vod_year, vod_month, vod_day, vod_hour, vod_minute
                                )
                                ...
                                if self.short_folder:
                                    ...
                                    processed_stream_folder = (
                                        vod_date_tz.strftime("%Y%m%d")
                                        + "_"
                                        + stream_title
                                        + "_"
                                        + vodsinfodic["videos"][0]["game"]
                                        + "_"
                                        + self.username
                                    )
                                    processed_stream_folder = "".join(
                                        x
                                        for x in processed_stream_folder
                                        if x.isalnum()
                                        or not x in ["/", "\\", ":", "?", "*", '"', ">", "<", "|"]
                                    )
                                ...
                                if self.make_stream_folder:
                                    processed_stream_path = (
                                        self.processed_path + "/" + processed_stream_folder
                                    )
                                else:
                                    processed_stream_path = self.processed_path

                                filename = (
                                    vod_date_tz.strftime("%Y%m%d_(%H-%M)")
                                    + "_"
                                    + vod_id
                                    + "_"
                                    + stream_title
                                ...
                                if len(os.path.join(self.recorded_path, filename)) >= 260:
                                    difference = (
                                        len(stream_title)
                                        - len(os.path.join(self.recorded_path, filename))
                                        + 250
                                    )
                                    if difference < 0:
                                        if self.warning_windows:
                                            if self.osCheck == SystemPlatform.WINDOWS:
                                                subprocess.call(
                                                    ...
                                                    + [
                                                        "echo",
                                                        "Path to stream is too long. (Max path length is 259 symbols) Title cannot be cropped, please check root path. Stream will not be processed.",
                                                    ]
                                                )
                                            else:
                                                subprocess.call(
                                                    " ".join(
                                                        ...
                                                        + [
                                                            ...
                                                            "-c",
                                                            "'" + "echo",
                                                            '"Path to stream is too long. (Max path length is 259 symbols) Title cannot be cropped, please check root path. Stream will not be processed."; exec bash'
                                                            + "'",
                                                        ]
                                                    ),
                                                    shell=True,
                                        ...
                                        if self.warning_windows:
                                            if self.osCheck == SystemPlatform.WINDOWS:
                                                subprocess.call(
                                                    ...
                                                    + [
                                                        "echo",
                                                        "Path to stream is too long. (Max path length is 259 symbols) Title will be cropped, please check root path.",
                                                    ]
                                                )
                                            else:
                                                subprocess.call(
                                                    " ".join(
                                                        ...
                                                        + [
                                                            ...
                                                            "-c",
                                                            "'" + "echo",
                                                            '"Path to stream is too long. (Max path length is 259 symbols) Title will be cropped, please check root path."; exec bash'
                                                            + "'",
                                                        ]
                                                    ),
                                                    shell=True,
                                ...
                                if len(os.path.join(processed_stream_path, filename)) >= 260:
                                    if self.short_folder == 1 or not self.make_stream_folder:
                                        difference = (
                                            len(stream_title)
                                            - len(os.path.join(processed_stream_path, filename))
                                            + 250
                                        )
                                    else:
                                        difference = int(
                                ...
                                try:
                                    os.rename(
                                        recorded_filename,
                                        os.path.join(self.recorded_path, filename),
                                    )
                                    recorded_filename = os.path.join(self.recorded_path, filename)
                                    processed_filename = os.path.join(
                                        processed_stream_path, filename
                                    )
                                except Exception as e:
                                    stream_title = str(info["status"])
                                    stream_title = "".join(
                                            ...
                                            subprocess.call(
                                                " ".join(
                                                    ...
                                                    + [
                                                        ...
                                                        "-c",
                                                        "'" + "echo",
                                                        '"An error has occurred. VOD and chat will not be downloaded. Please check them manually."; exec bash'
                                                        + "'",
                                                    ]
                                                ),
                                                shell=True,
                                ...
                                if self.chatdownload and filenameError == 0:
                                    if self.osCheck == SystemPlatform.WINDOWS:
                                        subtitles_window = (
                                            self.main_cmd_window + self.cmdstatecommand
                                        )
                                        if self.cmdstate == 3:
                                            subprocess.Popen(
                                                [
                                ...
                                if self.downloadVOD and filenameError == 0:
                                    ...
                                    if self.osCheck == SystemPlatform.WINDOWS:
                                        ...
                                        if self.cmdstate == 3:
                                            subprocess.Popen(
                                                ...
                                                + [
                                                    "-o",
                                                    os.path.join(self.recorded_path, vod_filename),
                                                ],
                                                stdout=None,
                                                stderr=None,
                                            )
                                            ...
                                            subprocess.call(
                                                " ".join(
                                                    ...
                                                    + [
                                                        ...
                                                        '"'
                                                        + os.path.join(
                                                            self.recorded_path, vod_filename
                                                        )
                                                        + '"'
                                                        + self.linuxstatecomma,
                                                    ]
                                ...
                                subprocess.call(
                                    " ".join(
                                        ...
                                        + [
                                            ...
                                            "-bsf:a",
                                            "aac_adtstoasc",
                                            '"' + processed_filename + '"' + self.linuxstatecomma,
                                        ]
                                    ),
                                    shell=True,
                                    stdout=subprocess.PIPE,
